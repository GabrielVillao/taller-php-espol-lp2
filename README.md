...
## Archivos agregados para el proyecto final

```text
├── tarea.php            → funciones de persistencia de tareas
├── tareas.php           → pantalla protegida para administrar tareas
├── usuarios.csv         → persistencia de usuarios
├── logs.txt             → intentos fallidos de ingreso
└── tareas_<cedula>.csv  → tareas privadas de cada usuario (se crea automáticamente)
```
